...
public class PokerHand
{
  ...
  public boolean isStraight()
  {
    ...
    return cards[0].getRank() == cards[cards.length - 1].getRank()-4;
  }
  ...
  public boolean isFullHouse()
  {
    sortNoSuit();
 // TODO Finish this method
    int countFirstCard = 0;
    int countSecondCard = 0;
    
    Card fristCard = cards[0];
    Card secondCard = cards[0];
    ...
    for (int i=0; i<cards.length; i++)
    {
        ...
        {
        	if (fristCard != cards[j])
        	{
        		secondCard = cards[j];
        	}
        }
    	if(fristCard.getRank() == cards[i].getRank())
    	{
    		countFirstCard++;
    	}
    	if(secondCard.getRank() == cards[i].getRank())
    	{
    		countSecondCard++;
    	}
    }
    
    if (countFirstCard == 2 && countSecondCard == 3)
    {
    	return true;
    }
    else if (countFirstCard == 3 && countSecondCard == 2)
    {
    	return true;
    }
    else
  ...
  /**
   * Returns the index of the minimal element to the right of position 'start',
   * where cards are ordered by rank, suit doesn't have to be in order.
   * 
   * @param start
   *          first index at which to look
   * @return index of the smallest element to the right of 'start'
  ...
  private int findIndexOfMinNoSuit(int start)
  {
    ...
    for (int j = start + 1; j < cards.length; ++j)
    {
      ...
      // if current has a lower rank than the current min, then it should come
      // first
      if (current.getRank() < min.getRank())
      {
    	  indexOfMin = j;
      }
//      else if (current.getRank() == min.getRank())
//      {
//    	  indexOfMin = j;
//      }
